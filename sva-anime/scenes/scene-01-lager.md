# Szene 01 — „Vorbereitung im Heerlager" (Test-Szene)

**Dauer:** ca. 11 Sekunden · **Format:** 9:16 · **Stil:** Anime-Cinematic, cel-shaded, filmisch
**Setting:** Osmanisches Heerlager am Vorabend der Schlacht, Dämmerung, Lagerfeuer, Zelte, Banner mit SVA-Wappen

**Figuren:**
- **Heerführer / Feldherr** — Sercan Selcuk Kaya (`assets/heerfuehrer-sercan.jpeg`), Anführer des Heeres
- **Krieger** — Spieler von SV Anadolu Limburg (`assets/team.png`)

---

## Shot 1 — Establishing (0:00–0:03)

**Bild (Keyframe-Prompt):**
> Anime cinematic, cel-shaded, vertical 9:16. A medieval Ottoman war camp at dusk,
> rows of tents, banners bearing a red-and-black circular crest ("SVA"), warriors
> gathered around a campfire, mountains and a darkening sky in the background,
> epic dramatic lighting, embers floating, high detail anime film still.

**Animation (image-to-video):** Langsamer Kamera-Push nach vorne durchs Lager, Funken
fliegen, Banner wehen im Wind.

**Untertitel:** *„Am Vorabend der Schlacht versammelte sich das Heer von Anadolu…"*

---

## Shot 2 — Zwei Krieger am Feuer (0:03–0:06)

**Bild (Keyframe-Prompt):**
> Anime cinematic, cel-shaded, vertical 9:16. Two Ottoman warriors in ornate armor
> worn over grey-and-white football jerseys (crest "SVA" on the chest), standing by
> a campfire holding wooden mugs, confident expressions, fur-trimmed capes, a large
> tent and Ottoman banners behind them, warm firelight on their faces.

> **Gesichts-Referenz:** zwei Spieler aus `assets/team.png` (Charakter-Referenz-Rolle).

**Animation:** Leichte Bewegung — einer hebt den Becher, der andere nickt; Feuer flackert.

**Untertitel:** *„Morgen reiten wir gegen die Feinde."*

---

## Shot 3 — Auftritt des Heerführers (0:06–0:09)

> **Hauptfigur: der Heerführer (Feldherr)** — gespielt von **Sercan Selcuk Kaya**.
> Er ist der Anführer des Heeres von Anadolu.

**Bild (Keyframe-Prompt):**
> Anime cinematic, cel-shaded, vertical 9:16. A commanding Ottoman general (the warlord)
> stepping forward through the war camp, ornate gold-and-crimson armor with a fur-lined
> cape over a grey-and-white "SVA" tabard, a crescent-crested helmet held under one arm,
> short black beard, intense confident gaze, full beard, warriors bowing around him,
> giant Ottoman banners and the SVA crest behind, torchlight, epic low-angle hero shot.

> **Gesichts-Referenz:** `assets/heerfuehrer-sercan.jpeg` (Charakter-Referenz-Rolle — Gesicht des Feldherrn).

**Animation:** Heroischer Low-Angle, langsamer Push nach oben; sein Umhang weht, er hebt den Blick,
Krieger im Hintergrund senken die Köpfe.

**Untertitel:** *„Heute Nacht schreiben wir Geschichte."*

---

## Shot 4 — Schlachtruf (0:09–0:11)

**Bild (Keyframe-Prompt):**
> Anime cinematic, cel-shaded, vertical 9:16. Extreme close-up of the warlord's face
> (same character), determined battle smile, war paint, drawing a curved sword skyward,
> sparks and embers, the whole army raising swords behind him, dramatic backlight,
> epic anime film still.

> **Gesichts-Referenz:** `assets/heerfuehrer-sercan.jpeg`.

**Animation:** Schwert reißt nach oben, Heer brüllt, Kamera-Shake auf den Schrei, Funkenregen.

**Untertitel:** *„Für Anadolu!"*

---

## Audio / Voiceover

> **Sprache: Japanisch (Anime-Voiceover/-Dialog), mit deutschen Untertiteln.**
> Die eingeblendeten Untertitel (oben in den Shots) sind die deutsche Übersetzung;
> gesprochen wird auf Japanisch im Anime-Stil.

| Shot | Untertitel (DE) | Gesprochen (JA, Romaji) |
|------|-----------------|--------------------------|
| 2 | „Morgen reiten wir gegen die Feinde." | „Ashita, teki ni mukatte susumu." |
| 3 | „Heute Nacht schreiben wir Geschichte." | „Konya, wareware ga rekishi o tsukuru." |
| 4 | „Für Anadolu!" | „Anadoru no tame ni!" |

- **Erzähler (tief, episch, Japanisch):** sinngemäß „Das Heer von Anadolu Limburg stand
  bereit. Die Nacht war kalt — doch ihr Mut brannte heller als jedes Feuer."
  (DE als Untertitel eingeblendet.)
- **Musik:** Epischer Taiko/Orchester-Anime-Battle-Track, anschwellend.

## Modell-Notizen (Higgsfield)

- **Keyframes:** anime-fähiges Image-Modell mit Gesichts-/Charakter-Referenz
  (z.B. `nano_banana_pro` oder `soul_2` mit Referenz-Media-Rolle).
- **Animation:** `kling3_0` (Multi-Shot/Motion) oder `kling3_0_turbo` (schnell), 9:16, 5 s je Shot.
- **Reihenfolge:** erst alle Keyframes bestätigen, dann image-to-video, dann Schnitt.
